...
class RNN_CVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = build_encoder('gru',
                                     emb_dim=self.emb_dim + cfg.EGCM_embeded_length,
                                     z_dim=z_dim,
                                     **E_args)
        self.decoder = build_decoder(embedding=self.word_emb,
                                     emb_dim=self.emb_dim + z_dim + cfg.EGCM_embeded_length,
                                     output_dim=n_vocab,
                                     h_dim=z_dim,
                                     **G_args)
        self.encoder_EGCM_mu = nn.Linear(cfg.EGCM_embeded_length + z_dim, z_dim)
        self.encoder_EGCM_logvar = nn.Linear(cfg.EGCM_embeded_length + z_dim, z_dim)
        self.linear = nn.Linear(cfg.EGCM_max_length, cfg.EGCM_embeded_length)
    ...
    def forward_encoder(self, inputs, feature):
        '''
        ...
        feature = feature.unsqueeze(1).expand(-1, inputs.size(1), -1)
        conditional_seq = torch.cat((inputs, feature), dim=-1)
        return self.encoder(conditional_seq)

    def forward_encoder_EGCM(self, z, feature):
        '''
        Inputs is batch of sentences: seq_len x mbsize
               or batch of soft sentences: seq_len x mbsize x n_vocab.
        '''
        conditional_seq = torch.cat((z, feature), dim=-1)
        return self.encoder_EGCM_mu(conditional_seq), self.encoder_EGCM_logvar(conditional_seq)
    ...
    def forward(self, sequence, eigen_CM, q_c='prior', sample_z=1):
        """
        ...
        # Encoder: sequences -> z
        feature = self.linear(eigen_CM)
        mbsize = sequence.size(0)
        mu, logvar = self.forward_encoder(sequence, feature)

        prior_z = self.sample_z_prior(sequence.size(0))
        mu2, logvar2 = self.forward_encoder_EGCM(prior_z, feature)
        ...
        # Decoder: (x,z,c) -> y
        dec_logits = self.forward_decoder(sequence, z, feature)
        return (mu, logvar),(mu2, logvar2), z, dec_logits

    def generate_sentences(self, mbsize, eigen_CM, z=None, eval_mode=True, **sample_kwargs):
        """
        ...
        if eval_mode:
            self.eval()
        feature = self.linear(eigen_CM)
        sentences = self.sample_G(mbsize, z, feature, **sample_kwargs)
        if eval_mode:
            self.train()
        return sentences, z

    def sample_G(self, mbsize, z, feature,
        ...
        for i in range(self.MAX_SEQ_LEN):
            ...
            logits, h = self.decoder.forward_sample(sampleSoftIx, sampleIx, z, h, feature)
