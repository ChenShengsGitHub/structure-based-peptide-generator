...
class GRUDecoder(nn.Module):
    ...
    def forward(self, x, z, feature):
        ...
        expanded_init_h = z.unsqueeze(1).expand(-1, seq_len, -1)

        # Construct input to RNN
        # TODO parametrize whether to concat z,c to decoder input or not?
        feature = feature.unsqueeze(1).expand(-1, seq_len, -1)
        dec_inputs = torch.cat([dec_inputs, expanded_init_h, feature], 2)

        # Compute outputs. # mbsize x seq_len x h_dim
        rnn_out, _ = self.rnn(dec_inputs, z.unsqueeze(0))
    ...
    def forward_sample(self, sampleSoft, sampleHard, z, h, feature):
        ...
        # apply skip connection
        if self.skip_connetions:
            output = self.skip_weight_x(output) + self.skip_weight_z(z)
